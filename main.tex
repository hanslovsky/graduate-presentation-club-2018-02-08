...
\begin{frame}
    ...
    \only<1>{
        \begin{tikzpicture}
            \node[] (diagram) {\includegraphics[width=0.6\textwidth]{fig/circuit-diagram.pdf}};
...
\begin{frame}
    ...
    % width=\factor\textwidth,%
    % height=\factor\textheight,%
    ]{\includegraphics[width=\textwidth]{vid/mesh-placeholder.jpg}}{vid/mesh.mp4}
\end{frame}
...
\begin{frame}
    ...
    % width=\factor\textwidth,%
    % height=\factor\textheight,%
    ]{\includegraphics[width=\textwidth]{vid/bigcat-synapses-placeholder.jpg}}{vid/bigcat-synapses.mp4}
\end{frame}
...
\begin{frame}
    ...
    \begin{textblock}{9.8}(6,0.5)
        \scriptsize%
        \begin{description}
              \item[The Lab:]{John Bogovic, Philipp Hanslovsky, Larissa Heinrich, Shelby Morris, Igor Pisarev, Stephan Saalfeld, Neil Thistlethwaite}
              \item[Visitors:]{Jan Funke, Constantin Pape}
              \item[OK:]{Tobias Pietzsch}
\end{description}
\end{textblock}
    % tbd
    % \begin{itemize}
